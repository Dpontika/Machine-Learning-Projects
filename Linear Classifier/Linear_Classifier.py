...
for fold in range(9):
    ...
    ttrain1 = np.array(ttrain) # Create copy
    for i in range(len(ttrain)):
        ttrain1[i] = new_dict[ttrain[i]]

    ttest1 = np.array(ttest) # Create copy
    for i in range(len(ttest)):
        ttest1[i] = new_dict[ttest[i]]

    # Pseudo-inverse method to find optimal weights that best separate the two classes in the training data 
